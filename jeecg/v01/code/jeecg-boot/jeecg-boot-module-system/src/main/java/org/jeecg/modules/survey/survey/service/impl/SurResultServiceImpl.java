 ...
 */
@Service
public class SurResultServiceImpl extends ServiceImpl<SurResultMapper, SurResult> implements ISurResultService {

    @Autowired
    private SurUserResultMapper userResultMapper;
    @Autowired
    private SurResultMapper resultMapper;
    @Autowired
    private SurProjectSurveyMapper projectSurveyMapper;
    @Autowired
    ...
    @Autowired
    private SurQuestionMapper questionMapper;
    @Autowired
    private SurQuestionChoiceMapper choiceMapper;
    @Autowired
    private SurSurveyProjectMapper surveyProjectMapper;
    @Autowired
    private SurQuestionProjectMapper questionProjectMapper;
    @Autowired
    private SurQuestionChoiceProjectMapper choiceProjectMapper;
    @Autowired
    ...
    public Boolean saveSurResult(SaveSurResultReq req) {
        ...
        if (surProject.getType().equals("测评")) {
            SurResult one = resultMapper.selectOne(new LambdaQueryWrapper<SurResult>().eq(SurResult::getProjectUid,req.getProjectId()).eq(SurResult::getUserUid,user.getId()));
            if (one != null) {
                return false;
            }
        }
        //如果是360度项目 则可能有多个答案
            ...
            SurUserResult userResult = userResultMapper.selectOne(new LambdaQueryWrapper<SurUserResult>().eq(SurUserResult::getProjectId, req.getProjectId()).eq(SurUserResult::getUserId, user.getId()).eq(SurUserResult::getEvaluatedId, req.getEvaluatedId()));
            if (userResult != null) {
                return false;
            }
        }
        //根据评价人id查询评价关系对象
        SurEvaluationUser evaluationUser = evaluationUserMapper.selectOne(new LambdaQueryWrapper<SurEvaluationUser>().eq(SurEvaluationUser::getEvaluatorId, user.getId()).eq(SurEvaluationUser::getUserId,req.getEvaluatedId()).eq(SurEvaluationUser::getProjectId, req.getProjectId()));
        //先将整个问卷的答案存入到result表中
        SurResult surResult = new SurResult();
        surResult.setFid(IdUtil.simpleUUID());
        surResult.setAnswer(req.getAnswer());
        surResult.setSurveyUid(req.getSurveyId());
        ...
        if (surProject.getType().equals("测评")){
            if (surProject.getSurveyCurrentId() != null){
                //查询出该问卷
                SurSurveyProject surSurveyProject = surveyProjectMapper.selectById(surProject.getSurveyCurrentId());
        ...
        while (entries.hasNext()) {
            ...
            if (value instanceof String) {
                SurUserResult userResult = SurUserResult.builder()
                        .phone(user.getPhone())
                        .surveyId(req.getSurveyId())
                        .userId(user.getId())
                        .projectId(req.getProjectId()).build();
                ...
                map1 = getChoiceMap(req.getSurveyId(), key);
                //遍历选项和分数的map
                if (map1 == null){
            ...
            if (value instanceof Boolean){
                ...
                map2 = getChoiceMap(req.getSurveyId(), key);
                if (map2 == null){
                    userResult.setBasicScore(0);
                }
                else {
            ...
            if (value instanceof Map){
                ...
                while (iterator.hasNext()) {
                    ...
                    map3 = getChoiceMap(req.getSurveyId(), entry.getKey());
                    if (map3 == null){
                        userResult.setBasicScore(0);
                    }
                    else {
                        //遍历选项和分数的map
//                    Iterator<Map.Entry<String, Object>> entries3 = map3.entrySet().iterator();
...
//匹配数据库中的分数
        ...
        SurQuestionProject surQuestion = questionProjectMapper.selectOne(new LambdaQueryWrapper<SurQuestionProject>().eq(SurQuestionProject::getSurveyUid, surveyId).eq(SurQuestionProject::getContent, key));
        Map<String, Object> map = new LinkedHashMap<>();
        //根据问题id查询数据库中的选项对象
        if (surQuestion != null){
            SurQuestionChoiceProject surQuestionChoice = choiceProjectMapper.selectOne(new LambdaQueryWrapper<SurQuestionChoiceProject>().eq(SurQuestionChoiceProject::getQuestionId, surQuestion.getId()));
            //获取选项数组
