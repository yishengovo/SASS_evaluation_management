...
public class ThirdLoginController {
	...
	@RequestMapping(value = "/getLoginUser/{token}/{thirdType}/{loginEnd}", method = RequestMethod.GET)
	@ResponseBody
	public Result<JSONObject> getThirdLoginUser(@PathVariable("token") String token,@PathVariable("thirdType") String thirdType,@PathVariable("loginEnd") Integer loginEnd) throws Exception {
		Result<JSONObject> result = new Result<JSONObject>();
	...
	public Result<String> bindingThirdPhone(@RequestBody JSONObject jsonObject) {
		...
		Object captchaCache = redisUtil.get("bindWechatOpenSms"+phone);
		...
		if(sysUser != null){
			...
			sysUser.setWechatNickname(sysThirdAccountService.getOneBySysUserId(sysUser.getId(),"wechat_open").getRealname());
			sysUserService.updateById(sysUser);
			result.setMessage("绑定成功!");
		}else{
			// 不存在手机号，创建用户
			sysUser = sysThirdAccountService.createUser(phone,thirdUserUuid);
			sysUserService.addUserWithRole(sysUser,"1581159129809661954");
			sysUser.setWechatNickname(sysThirdAccountService.getOneBySysUserId(sysUser.getId(),"wechat_open").getRealname());
			sysUserService.updateById(sysUser);
			result.setMessage("账号创建成功，初始密码为手机号!");
		}
		String token = saveToken(sysUser);
		result.setSuccess(true);
		result.setResult(token);
	...
	@PostMapping("/bindWechatOpen")
	@ResponseBody
	public Result<String> bindWechatOpen(HttpServletRequest request, @RequestBody JSONObject jsonObject){
		String uuid = jsonObject.getString("uuid");
		String userName = JwtUtil.getUserNameByToken(request);
		SysUser user = sysUserService.getUserByName(userName);
		if(user==null){
			return Result.error("该用户尚未登录");
		}
		try{
			sysThirdAccountService.updateThirdUserId(user, uuid);
			user.setWechatNickname(sysThirdAccountService.getOneBySysUserId(user.getId(),"wechat_open").getRealname());
			sysUserService.updateById(user);
		}catch (Exception e){
			return Result.error("绑定失败");
		}
		return Result.ok(user.getWechatNickname());

	}

	@DeleteMapping("/cancelBindWechatOpen")
	@ResponseBody
	public Result<String> cancelWechatOpen(HttpServletRequest request){
		String userName = JwtUtil.getUserNameByToken(request);
		SysUser user = sysUserService.getUserByName(userName);
		SysThirdAccount sysThirdAccount = sysThirdAccountService.getOneBySysUserId(user.getId(),"wechat_open");
		if(sysThirdAccount!=null){
			sysThirdAccountService.removeById(sysThirdAccount);
		}
