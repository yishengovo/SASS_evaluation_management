...
//		this.save(user);  //保存角色的时候已经添加过一次了
	...
	@Override
	public Boolean setUserEmail(SysUser user, String email) {
		user.setEmail(email);
	...
	@Override
	public String getUserNameByTenantId(String tenantId) {
		QueryWrapper<SysUser> queryWrapper = new QueryWrapper<>();
		queryWrapper.eq("rel_tenant_ids", tenantId);
		SysUser sysUser = userMapper.selectOne(queryWrapper);
		return sysUser.getUsername();
	}

	@Override
	public Boolean setUserPhone(SysUser user, String phone) {
		user.setPhone(phone);
		return userMapper.updateById(user)>0;
	}

	@Override
	public Boolean updateIntegral(String userId, Integer integral) {
	...
	public String getTenantNameByUserId(String userId) {
		...
		SysUser sysUser = userMapper.selectById(userId);
		//通过该用户id获取对应的租户
		SysTenant sysTenant = sysTenantMapper.selectById(sysUser.getRelTenantIds());
		return sysTenant.getName();
	}

	@Override
	public Boolean putUserIntegral(String userId, Integer integral) {
		SysUser sysUser = userMapper.selectById(userId);
		sysUser.setIntegral(integral);
		return userMapper.updateById(sysUser)>0;
	}
