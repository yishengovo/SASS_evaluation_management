...
//                json.put("avatar", item.getSysUser().getAvatar());
	...
	public Result<JSONObject> userRegister(@RequestBody JSONObject jsonObject, SysUser user) {
		...
		Object code = redisUtil.get("registPhone"+phone);
		...
		try {
            ...
            user.setRelTenantIds(Integer.toString(sysTenant.getId()));
			sysUserService.addUserWithRole(user,"1581159129809661954");//默认临时角色 test
            ...
            //给注册的用户设置saas角色
//            String roleId = sysRoleService.getIdByRoleCode("saas");
//            SysUserRole sysUserRole = new SysUserRole();
//            sysUserRole.setUserId(user.getId());
//            sysUserRole.setRoleId(roleId);
//            sysUserRoleService.insertSysUserRole(sysUserRole);
            redisUtil.del("registPhone"+phone);
			result.success("注册成功");
		} catch (Exception e) {
...
//	}
	...
	public Result<SysUser> passwordChange(@RequestParam(name="username")String username,
        ...
        Object object= redisUtil.get("forgetphone"+phone);
        if(null==object) {
        ...
        if (sysUser == null) {
            ...
            redisUtil.del("forgetphone"+phone);
            return result;
        }
    }
    ...
    public Result<JSONObject> getUserInfoByToken(HttpServletRequest request, @RequestParam(name = "token", required = false) String token) {
        try {
            Result<JSONObject> result = new Result<JSONObject>();
            String username = null;
            // 如果没有传递token，就从header中获取token并获取用户信息
            ...
            log.debug(" ------ 通过令牌获取用户信息，当前用户： " + username);

            // 根据用户名查询用户信息
            SysUser sysUser = sysUserService.getUserByName(username);

            getUserInfo(sysUser,result);
    ...
    public Result<String> getEmailCode(HttpServletRequest request, @RequestBody JSONObject jsonObject){
        ...
        Object object = redisUtil.get("bindEmail"+email);
        if (object != null) {
        ...
        String username = JwtUtil.getUserNameByToken(request);
        SysUser sysUser = sysUserService.getUserByName(username);
        if(oConvertUtils.isNotEmpty(sysUser.getEmail())){
            result.error500("该用户已绑定过邮箱！");
            result.setSuccess(false);
            return result;
        }
        //判断改邮箱是否被用过
        sysUser = sysUserService.getUserByEmail(email);
        if(oConvertUtils.isNotEmpty(sysUser)){
            result.error500("该邮箱已被绑定，请更换邮箱！");
            result.setSuccess(false);
            return result;
        }
        ...
        String es_title = "【朗新天霁】绑定邮箱验证";
        String es_content = "尊敬的用户,您好:\n" +
                "    您正在朗新天霁进行邮箱绑定的操作，本次请求的邮件验证码是："+captcha+" (为了保证您账号的安全性，请您在5分钟内完成验证).\n" +
                "本验证码5分钟内有效，请及时输入。\n" +
                "\n" +
                "    为保证账号安全，请勿泄漏此验证码。\n" +
                "    祝在【朗新天霁】使用愉快！\n" +
        ...
        redisUtil.set("bindEmail"+email, captcha, 300);
        result.setSuccess(true);
        return result;
    }
     ...
     * @description: 绑定邮箱
     * @author: GGB
     * @date: 2022/10/21 17:25
     * @param: jsonObject
     * @param: user
     * @return: Result<JSONObject>
     **/
    @PostMapping("/bindEmail")
    public Result<JSONObject> bindEmail(HttpServletRequest request, @RequestBody JSONObject jsonObject, SysUser user){
        Result<JSONObject> result = new Result<JSONObject>();
        String email = jsonObject.getString("email");
        String emailCode = jsonObject.getString("emailCode");
        System.out.println("dasdasdasdas"+emailCode);
        Object code = redisUtil.get("bindEmail"+email);
        if(null == code){
            result.error500("邮箱验证码失效，请重新获取");
            result.setSuccess(false);
            return result;
        }
        ...
        try {
            ...
            if(sysUserService.setUserEmail(user,email)){
                result.success("邮箱绑定成功");
            }else {
                result.error500("邮箱绑定失败");
                result.setSuccess(false);
            }
        } catch (Exception e) {
     ...
     * @return: Result<String>
     **/
    @PostMapping("/getRebindEmailCode")
    public Result<String> getRebindEmailCode(HttpServletRequest request, @RequestBody JSONObject jsonObject){
        Result<String> result = new Result<String>();
        String email = jsonObject.get("email").toString();
        //为1代表获取原有的邮箱验证码，为2代表获取需要换绑的邮箱验证码
        ...
        if(rebindFlag==1){
            if(!sysUser.getEmail().equals(email)){
                result.error500("该邮箱非该用户的邮箱!");
                result.setSuccess(false);
                return result;
            }
            object = redisUtil.get("rebindBeforeEmail"+email);
        }else if(rebindFlag==2){
            object = redisUtil.get("rebindEmail"+email);
        }else {
            object = redisUtil.get("forgetemail"+email);
        }
        ...
        if(rebindFlag==2){
            //用户上一步获取原有邮箱的验证码后20分钟内可以继续进行换绑邮箱操作

            String beforeEmail = sysUser.getEmail();
            object = redisUtil.get("beforeEmailVaild"+beforeEmail);
            if(object == null){
                result.error500("长时间未操作，原邮箱验证已失效！");
                result.setSuccess(false);
                return result;
            }
            //判断改邮箱是否被用过
        ...
        if(rebindFlag == 1){
            redisUtil.set("rebindBeforeEmail"+email, captcha, 300);
            redisUtil.set("beforeEmailVaild"+email, captcha, 1200);
        }else if(rebindFlag == 2){
            redisUtil.set("rebindEmail"+email, captcha, 300);
        }else {
            redisUtil.set("forgetemail"+email, captcha, 300);
        }
    ...
    public Result<JSONObject> rebindEmail(HttpServletRequest request, @RequestBody JSONObject jsonObject, SysUser user){
        Result<JSONObject> result = new Result<JSONObject>();
        String email = jsonObject.getString("email");
        String emailCode = jsonObject.getString("emailCode");
        Integer rebindFlag = jsonObject.getInteger("rebindFlag");
        Object code = null;
        if(rebindFlag == 1){
            code = redisUtil.get("rebindBeforeEmail"+email);
        }else {
            code = redisUtil.get("rebindEmail"+email);
        }
        ...
        if(rebindFlag == 1){
            redisUtil.del("rebindBeforeEmail"+email);
            return result.ok("邮箱验证码正确");
        }
        try {
            //获取当前登录用户
            String username = JwtUtil.getUserNameByToken(request);
    ...
    public Result<SysUser> passwordChangeByEmail(@RequestParam(name="username")String username,
                                          @RequestParam(name="password")String password,
                                          @RequestParam(name="emialcode")String emialcode,
                                          @RequestParam(name="email") String email) {
        Result<SysUser> result = new Result<SysUser>();
        if(oConvertUtils.isEmpty(username) || oConvertUtils.isEmpty(password) || oConvertUtils.isEmpty(emialcode)  || oConvertUtils.isEmpty(email) ) {
            result.setMessage("重置密码失败！");
            result.setSuccess(false);
            return result;
        }
        ...
        SysUser sysUser=new SysUser();
        Object object= redisUtil.get("forgetemail"+email);
        if(null==object) {
            result.setMessage("邮箱验证码失效！");
            result.setSuccess(false);
            return result;
        }
        if(!emialcode.equals(object.toString())) {
        ...
        if (sysUser == null) {
            ...
            result.setSuccess(true);
            result.setMessage("密码重置完成！");
            redisUtil.del("forgetemail"+email);
            return result;
        }
    }
    ...
    private Result<JSONObject> getUserInfo(SysUser sysUser, Result<JSONObject> result) {
        String syspassword = sysUser.getPassword();
        String username = sysUser.getUsername();
        // 获取用户部门信息
        JSONObject obj = new JSONObject();
