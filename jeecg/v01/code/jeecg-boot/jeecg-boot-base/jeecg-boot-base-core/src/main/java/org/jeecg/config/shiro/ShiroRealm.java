...
import org.jeecg.common.exception.JeecgBoot412Exception;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        } catch (JeecgBoot411Exception e){
            JwtUtil.responseError(SpringContextUtils.getHttpServletResponse(),411,e.getMessage());
            e.printStackTrace();
            return null;
        } catch (JeecgBoot412Exception e){
            JwtUtil.responseError(SpringContextUtils.getHttpServletResponse(),412,e.getMessage());
            e.printStackTrace();
            return null;
    ...
    public LoginUser checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        else if(hostUserName == null){

            throw new JeecgBoot411Exception("不存在该租户");
        }
        else if(!hostUserName.equals(username)){
            throw new JeecgBoot412Exception("用户租户域名变更，请重新登录!");
        }
